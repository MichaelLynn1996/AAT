...
base_exp_dir=./exp/${model}-${dataset}-f$fstride-t$tstride-imp$imagenetpretrain-asp$audiosetpretrain-b$batch_size-lr${lr}-prompt_mlp_spatial
...
do
  ...
  --dataset_mean ${dataset_mean} --dataset_std ${dataset_std} --audio_length ${audio_length} --noise ${noise} --finetune head prompt mlp spatial
done
