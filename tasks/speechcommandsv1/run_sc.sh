...
dataset_mean=-6.785136
dataset_std=5.496325
audio_length=128
noise=True
...
exp_dir=./exp/test-${dataset}-f$fstride-t$tstride-p$imagenetpretrain-b$batch_size-lr${lr}-prompt_mlp_spatial
...
--tstride $tstride --fstride $fstride --imagenet_pretrain $imagenetpretrain --audioset_pretrain $audiosetpretrain > $exp_dir/log.txt --finetune prompt head mlp spatial
